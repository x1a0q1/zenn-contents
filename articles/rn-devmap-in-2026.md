...
flowchart TB
  ...
  Shared[画面をまたいで共有する状態<br/>テーマ / 認証UI / アプリ設定] --> Zustand[Zustand]
  Local[画面内で閉じる状態<br/>入力途中 / 開閉状態] --> ReactState[useState / useReducer]
...
つまり、RNアプリでの状態はまず大きく2種類に分け、Client Stateはさらに共有範囲で置き場所を決めます。
...
:::message
TanStack Query自体はRNでもそのまま動きますが、「アプリがフォアグラウンドへ戻った」「ネットワークへ再接続した」というモバイル固有の状態は別途つなぎます。実プロジェクトでは`AppState`と`focusManager`、NetInfoまたは`expo-network`と`onlineManager`を連携させるのが定番です。詳しくは[TanStack QueryのReact Nativeガイド](https://tanstack.com/query/latest/docs/framework/react/react-native)を参照してください。
:::
